...
fn issue_269_270_comment_nodes_and_raw_tag_name() {
	...
	assert!(matches!(
		comment.node_type(),
		node_html_parser::NodeType::Comment
	));
...
fn issue_269_270_query_selector_comment() {
  ...
  <body>
	...
	fn dfs_node(node: &node_html_parser::Node, out: &mut Option<String>) {
		match node {
			node_html_parser::Node::Comment(c) => {
				if c.text.contains("Some comment here.") {
					*out = Some(c.text.clone());
					return;
				}
			}
	...
	assert!(
		found.is_some(),
		"should find comment node with expected text"
	);
}
