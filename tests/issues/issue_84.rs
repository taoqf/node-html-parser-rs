...
fn issue_84_query_selector_variations() {
	...
	assert!(std::ptr::eq(
		root.query_selector("span.a").unwrap(),
		span_ab
	));
	assert!(std::ptr::eq(
		root.query_selector("span.b").unwrap(),
		span_ab
	));
	assert!(std::ptr::eq(
		root.query_selector("span.a.b").unwrap(),
		span_ab
	));
	assert!(std::ptr::eq(
		root.query_selector("#id .b").unwrap(),
		span_ab
	));
	assert!(std::ptr::eq(
		root.query_selector("#id span").unwrap(),
		span_ab
	));
	assert!(std::ptr::eq(
		root.query_selector("[data-id=myid]").unwrap(),
		a
	));
	assert!(std::ptr::eq(
		root.query_selector("[data-id=\"myid\"]").unwrap(),
...
fn issue_84_query_selector_all_variations() {
	let root =
		parse("<a id=\"id\"><div><span class=\"a b\"></span><span></span><span></span></div></a>");
	let a = root.query_selector("a").unwrap();
	...
	assert!(std::ptr::eq(
		root.query_selector_all("span.a")[0],
		first_span
	));
	assert!(std::ptr::eq(
		root.query_selector_all("span.b")[0],
		first_span
	));
	assert!(std::ptr::eq(
		root.query_selector_all("span.a.b")[0],
		first_span
	));
	assert!(std::ptr::eq(
		root.query_selector_all("#id .b")[0],
		first_span
	));
	// #id span -> 3 spans
